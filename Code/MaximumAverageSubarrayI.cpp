...
int main(){
    ...
    // setup
    double  windowavg   {std::numeric_limits<double>::min()};

    // calculating the first window average
    for(int i = 0; i<k; ++i) {windowavg += static_cast<double>(nums[i])/static_cast<double>(k);}
    double  runningavg  {windowavg};

    // going through the rest
    for(int i = k; i < nums.size(); ++i){
        // adding head 
        runningavg += static_cast<double>(nums[i])/static_cast<double>(k);

        // substracting tail
        runningavg -= static_cast<double>(nums[i-k])/static_cast<double>(k);

        // finding the bigger-value
        windowavg = windowavg > runningavg ? windowavg : runningavg;
    }

    // printing
    cout << format("largest-average = {}\n", windowavg);
