...
// main-file ===================================================================
string fCompareStrings(const string& runningstring, 
                       const string& finaloutput)
{
    
    // copying values
    auto a  {runningstring};
    auto b  {finaloutput};
    
    // init
    if (finaloutput.size() == 0)    {return runningstring;}
    
    // going through characters one after the other
    auto numminchars    = std::min(a.size(), b.size());

    for(int i = 0; i<numminchars; ++i){
        if (a[i] < b[i])            {return a;}
        else if (a[i] > b[i])       {return b;}
        else                        {continue;}
    }
    
    // if we're here, it means that the first minchars are the same
...
void foo(TreeNode*  root,
    ...
    // adding current character to string 
    runningstring = std::string(1, static_cast<char>(root->val + 'a')) + runningstring;
    
    // calling the function on the two children
    if (root->left == nullptr && root->right == nullptr){
        finaloutput = fCompareStrings(runningstring, finaloutput);
    }
    else{
        // calling the children
        foo(root->left,     runningstring, finaloutput);
        foo(root->right,    runningstring, finaloutput);
    }
