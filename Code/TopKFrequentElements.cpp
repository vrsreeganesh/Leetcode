...
int main(){
    ...
    // input- configuration
    auto nums   {vector<int>{4,1,-1,2,-1,2,3}};
    auto k      {2};
    ...
    for(const auto& num: nums){
        if (hist.find(num) == hist.end())   {hist[num] = 1;}
        else                                {++hist[num];}
    }

    // creating vector pair
    vector<vector<int>> kv;
    for(const auto& x: hist)        {kv.emplace_back(vector<int>{x.first, x.second});}

    // building the sorting lambda
    auto sortinglambda  = [](const auto& a, const auto& b)  {return a[1] > b[1];};

    // since we want just the first k-elements, use partial sort
    std::partial_sort(kv.begin(), kv.begin() + k, kv.end(), sortinglambda);

    // building the final output
    auto finaloutput    {vector<int>{}};
    for(int i = 0; i<k; ++i)    {finaloutput.push_back(kv[i][0]);}
