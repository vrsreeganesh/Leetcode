...
int main(){
    ...
    // setup
    auto runningsum     {0};
    auto binaryflag     {false};
    std::function<void(const TreeNode*, int)> fRunSum = [&fRunSum,
                    &targetSum,
                    &binaryflag](
        const TreeNode*   root,
        int         runningsum
        ...
        // sending it back
        if (root        == nullptr)     return;
        if (binaryflag  == true)        return;

        // adding current val to running sum
        runningsum += root->val;

        // checking if there are children 
        if (root->left || root->right){
            if (root->left)     fRunSum(root->left,     runningsum);
            if (root->right)    fRunSum(root->right,    runningsum);
        }
        else{
            // in this case, we check the running sum
            if (runningsum == targetSum){ binaryflag = true;}
        }
