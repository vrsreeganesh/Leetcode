...
int main(){
    ...
    for(const auto& x: arr){
        if (histogram.find(x) == histogram.end())   {histogram[x] = 1;}
        else                                        {++histogram[x];}
    }

    // building a set out of the histogram
    std::set<int> uniquecounts;
    bool finaloutput    {true};
    int uniquecounts_prevsize = 0;
    for(const auto& x: histogram){
        uniquecounts.insert(x.second);
        if (uniquecounts.size() == uniquecounts_prevsize)   {finaloutput = false; break;}
        uniquecounts_prevsize = uniquecounts.size();
    }

    // printing 
    cout << format("histogram.size() = {}\n", histogram.size());
    cout << format("finaloutput = {}\n", finaloutput);
    cout << format("uniquecounts = "); for(const auto& x: uniquecounts) {cout << x << ",";}
