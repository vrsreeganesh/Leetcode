...
int main(){
    ...
    std::function<void(const TreeNode*, int)> foo = [&foo,
                           &levelaverages,
                           &levelcount](
        const TreeNode* root,
        int       currentlevel){
        ...
        // adding the sum to the current-value
        while (levelaverages.size() < currentlevel+1)
        {
            // increasing size
            levelaverages.push_back(0);
            levelcount.push_back(   0);
        }

        // adding to sum 
        levelaverages[currentlevel] = \
            levelaverages[currentlevel] * \
            (static_cast<double>(levelcount[currentlevel])/static_cast<double>(levelcount[currentlevel]+1)) + \
            (static_cast<double>(root->val) / (levelcount[currentlevel] + 1));

        // increasing count value
        ++levelcount[currentlevel];

        // moving left 
        if (root->left)     {foo(root->left, currentlevel);}

        // moving right
        if (root->right)    {foo(root->right, currentlevel);}
    ...
    // returning level avaregaes
    cout << format("level-averages = {}\n", levelaverages);
