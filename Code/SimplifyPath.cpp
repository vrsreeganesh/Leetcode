...
int main(){
    ...
    while (i < path.size()){
        ...
        // moving until end of string or a /
        string temp;
        while(path[i]!='/' && i<path.size()){
            temp += std::string(1,path[i++]);
        }

        // continuing if temp size is zero
        if (temp.size()==0)     {++i; continue;}

        // evaluating the current string
        if (temp == ".")        {++i; continue;}
        else if (temp == "..")  {
            if (stack.size()!=0) {stack.pop_back();}
            ++i;}
        else    {stack.push_back(temp); ++i;}
    }


    // reconstructing final path
    string finalOutput;

    // reconstructing
    for(auto x: stack)  {finalOutput += "/" + x;}

    // size thing
    if (finalOutput.size() == 0) {finalOutput += "/";}
