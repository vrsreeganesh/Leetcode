...
int main(){
    ...
    // setup
    auto finaloutput        {static_cast<long long>(0)};
    auto leftboundary       {- std::pow(2, 31)};
    auto rightboundary      {-1 + std::pow(2, 31)};
    
    // building final output
    while(x){
        
        finaloutput = finaloutput*10 + x%10;
        x           = x/10;
        if (finaloutput < leftboundary || finaloutput > rightboundary)    {return 0;}    // checking the cases
    }
