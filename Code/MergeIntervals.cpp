...
int main(){
    ...
    vector<int> runninginterval {intervals[0][0], intervals[0][1]};
    for(int i = 1; i<intervals.size(); ++i){
        if (runninginterval[1] < intervals[i][0]){
            finalOutput.push_back(runninginterval);                             // pushing the results
            runninginterval = intervals[i];                                     // getting new running interfval
        }
        else    {runninginterval[1] = max(runninginterval[1], intervals[i][1]);}
    }
    finalOutput.push_back(runninginterval);                                     // pushing the results
