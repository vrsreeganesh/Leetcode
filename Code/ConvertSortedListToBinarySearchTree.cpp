...
// main-file ===================================================================
void foo(TreeNode*          parent, 
        int                parentdirection,
        vector<int>&       nums,
        int                left,
        int                right)
{
    // sending it back if right is less than left
    ...
    // connecting current to parent node
    if (parentdirection == -1)      parent->right = midnode;
    else if (parentdirection == 1)  parent->left = midnode;

    // going left
    foo(midnode, 1, nums, left, mid-1);

    // going right
    foo(midnode, -1, nums, mid+1, right);
...
int main(){
    ...
    // sending it back if the list is empty
    if (head == nullptr)        {cout << format("final-output = \n");}

    // making a vector out of the whole list
    auto traveller = head;
    vector<int> nums;
    while(traveller){
        nums.push_back(traveller->val);
        traveller = traveller->next;
    }

    // calling the previous function ito this
    TreeNode* dummy = new TreeNode(-1);
    
    // calling the function
    foo(dummy, -1, nums, 0, nums.size()-1);
