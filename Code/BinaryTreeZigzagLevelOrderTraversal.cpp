       ...
       int                                  level)
{
    ...
    // increasing size of the thing
    while (finalOutput.size()<level+1)  {finalOutput.push_back(std::vector<int>());}

    // appending to value
    if (finalOutput.size() < level+1 )  {finalOutput[level] = std::vector<int>(root->val);}
    else{
        // appending based on level
        if (level%2 == 0) 
            finalOutput[level].push_back(root->val);
        else  
        {
            std::vector<int> temp {root->val};
            for(auto x: finalOutput[level])     {temp.push_back(x);}
            finalOutput[level] = temp;
        }     
    }

    // calling 
    f(root->left, finalOutput, level);
    f(root->right, finalOutput, level);
...
int main(){
    ...
    f(root, finalOutput, -1);

    // sending back final output
