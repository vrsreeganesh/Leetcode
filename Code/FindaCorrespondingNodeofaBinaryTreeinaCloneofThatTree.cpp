...
int main(){
    ...
    // setup
    auto    finaloutput     {static_cast<TreeNode*>(nullptr)};
    auto    found       {false};

    // setting up lambda
    std::function<void(const TreeNode*, TreeNode*)> 
    seachLambda = [&seachLambda,
                &target,
                &found,
                &finaloutput](
            const   TreeNode*   curr_root,
            TreeNode*   curr_root_parallel
        )
    {
        // quitting if already found
        if(found == true)               {return;}

        // checking if current-value is target
        if(curr_root == target)         {finaloutput = curr_root_parallel; found = true; return;}

        // passing control to the children
        if (curr_root->left != nullptr) {seachLambda(curr_root->left, curr_root_parallel->left);}
        if (curr_root->right != nullptr) {seachLambda(curr_root->right, curr_root_parallel->right);}
