...
int main(){
    ...
    // setup
    vector<vector<int>> tempindexstack;
    auto finaloutput    {vector<int>(temperatures.size(), 0)};

    // going through the temperatures
    for(int i = 0; i<temperatures.size(); ++i){

        // fetching current-element
        auto& curr  = temperatures[i];

        while (tempindexstack.size() != 0 && tempindexstack.back()[0] < curr){

            auto& tempp             = tempindexstack.back()[0];     // fetching top-temperature
            auto& indexx            = tempindexstack.back()[1];     // fetching top-index
            finaloutput[indexx]     = (i - indexx);                 // calculating distance
            tempindexstack.pop_back();                              // popping back

        }
        
        // pushing element-index pair into the stack
        tempindexstack.push_back(vector<int>{curr, i});
    }
