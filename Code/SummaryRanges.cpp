...
int main(){
    ...
    // input- configuration
    auto nums   = vector<int>({0,1,2,4,5,7});
    
    // trivial-case
    if (nums.size() == 0)   {cout << format("final-output = {}\n", vector<string>()); return 0;}
    ...
    while (p2<nums.size()){
        ...
        // checking prev element
        if (nums[p2] <= 1 + nums[p2-1])   {++p2;}
        else{
            if (p1!=p2-1)   {finalOutput.push_back(std::to_string(nums[p1]) + arrow + std::to_string(nums[p2-1]));}
            else            {finalOutput.push_back(std::to_string(nums[p2-1]));}
            p1 = p2++;
        }    
    }

    // checking if p1 and p2 areright ne
    if (p1!=p2-1)   {finalOutput.push_back(std::to_string(nums[p1]) + arrow + std::to_string(nums[p2-1]));}
    else            {finalOutput.push_back(std::to_string(nums[p2-1]));}
