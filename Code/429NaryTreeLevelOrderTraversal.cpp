...
struct NodeLevelPair
{
    Node*   ptr;
    int     level;
    NodeLevelPair() = default;
    NodeLevelPair(Node*   ptr_, int level_): ptr(ptr_), level(level_) {}
};
...
int main(){
    ...
    auto    pipe    {std::deque<NodeLevelPair>()};
    pipe.push_back(NodeLevelPair({root, 0}));
    ...
    {
        // popping front
        auto    curr    {pipe.front()}; pipe.pop_front();
        auto    curr_level  {curr.level};

        // writing to final output
        if (curr_level < finaloutput.size())    {finaloutput[curr_level].push_back(curr.ptr->val);}
        else    {
            finaloutput.push_back(std::vector<int>({curr.ptr->val}));
        }

        // printing final output
        cout << format("curr->val = {}, finaloutput = {}\n", curr.ptr->val, finaloutput);

        // adding children 
        for(const   auto&   child:  curr.ptr->children)
            pipe.push_back({child, curr_level + 1});
    }
