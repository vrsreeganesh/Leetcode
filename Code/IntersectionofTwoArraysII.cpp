...
int main(){
    ...
    // setup
    unordered_map<int, int> histogram1; 
    unordered_map<int, int> histogram2; 

    // lambda to add to histogram
    auto addtohistogram = [](unordered_map<int, int>& histogram, int entryvalue){
        if (histogram.find(entryvalue) == histogram.end())  {histogram[entryvalue] = 1;}
        else                                                {++histogram[entryvalue];}
    };

    // building the histogram 
    for(const auto x: nums1)    {addtohistogram(histogram1, x);}
    for(const auto x: nums2)    {addtohistogram(histogram2, x);}

    // going through the first histogram
    vector<int> finaloutput;
    for(const auto kvp: histogram1){

        // splitting the pair
        auto& keyvalue  {kvp.first};
        auto& value     {kvp.second};

        // checking if it is there in the second histogram
        if(histogram2.find(keyvalue) != histogram2.end()){
            auto countforthis   {std::min(value, histogram2[keyvalue])};
            auto tempvector     {vector<int>(countforthis, keyvalue)};
            for(int i = 0; i < countforthis; ++i)   {finaloutput.push_back(keyvalue);}
        }
