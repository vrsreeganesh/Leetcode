...
int main(){
    ...
    // setup
    auto distance = [](auto argx){return std::sqrt(argx[0]*argx[0] + argx[1]*argx[1]);};

    // building heap
    vector<std::pair<int, double>> indexToDistance;

    // Basis on which the heap is built
    auto heap_comparator    = [](const auto& pair_a, const auto& pair_b){
        return pair_a.second < pair_b.second;
    };

    // making heap
    std::make_heap(indexToDistance.begin(), indexToDistance.end(), heap_comparator);

    //  lambda-function for pushing to heap
    auto pushHeap   = [&indexToDistance, &heap_comparator, &k](auto curr_pair){
        
        indexToDistance.push_back(curr_pair);
        std::push_heap(indexToDistance.begin(),indexToDistance.end(),heap_comparator);

        if (indexToDistance.size() > k){
            std::pop_heap(indexToDistance.begin(),indexToDistance.end(),heap_comparator);
            indexToDistance.pop_back();
        }
    };
    ...
    for(int i = 0; i<points.size(); ++i){
        ...
        // making current-entry
        auto curr   {std::pair{i, distance(points[i])}};
    ...
    auto finaloutput    {vector<vector<int>>{}};
    for(int i = 0; i<indexToDistance.size(); ++i)
        finaloutput.push_back(points[indexToDistance[i].first]);
