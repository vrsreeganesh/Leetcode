...
int main(){
    ...
    // setup
    std::function<bool(TreeNode*, TreeNode*)> fCompareTrees = [&fCompareTrees](
        ...
        // going through the next set of branches
        return (fCompareTrees(p->left, q->left) && fCompareTrees(p->right, q->right));
    };

    // going through the two nodes
    bool finalOutput = fCompareTrees(p, q);
