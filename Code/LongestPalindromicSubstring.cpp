...
int main(){
    ...
    for(int i = 0; i<s.size(); ++i){
        ...
        // checking odd-palindromes from here
        auto oddlength      {1};
        auto boundary       {1};
        string  oddstring   {s[i]};

        while(i - boundary >= 0 && i + boundary < s.size() && s[i-boundary] == s[i+boundary]){
            oddlength += 2;                                         // updating length
            oddstring = s.substr(i-boundary, 2*boundary+1);         // subsetting
            ++boundary;                                             // updating boundary
        }
        
        // checking even-palindromes from here
        auto evenlength     {0};
        boundary            = 1;
        auto evenstring     {string("")};

        while(i+1-boundary >= 0 && i+boundary < s.size() && s[i+1-boundary] == s[i+boundary]){
            evenlength += 2;                                        // updating length
            evenstring = s.substr(i+1-boundary, 2*boundary);        // subsetting
            ++boundary;                                             // updating boundary
        }
        
        // updating largest-string
        largestpalindrome = oddlength > largestpalindrome.size()    ? oddstring : largestpalindrome;
        largestpalindrome = evenlength > largestpalindrome.size()   ? evenstring : largestpalindrome;
