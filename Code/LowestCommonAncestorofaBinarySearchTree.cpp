...
void foo(const TreeNode*        root,
         const int              smallervalue, 
         const int              largervalue,
         TreeNode*&             finalanswer){
    
    if (root== nullptr)                 return;
    if (finalanswer != nullptr)         return;

    // checking current value
    if (smallervalue <= root->val && root->val <= largervalue) {finalanswer = root;}
    else if (root->val < smallervalue)                               {foo(root->right,   smallervalue, largervalue, finalanswer);}
    else if (root->val > largervalue)                                {foo(root->left,    smallervalue, largervalue, finalanswer);}
...
int main(){
    ...
    // setup
    const auto  smallervalue        {std::min(p->val, q->val)};
    const auto  largervalue         {std::max(p->val, q->val)};
    auto        finalanswer         {static_cast<TreeNode*>(nullptr)};

    //calling the function
    foo(root, smallervalue, largervalue, finalanswer);
