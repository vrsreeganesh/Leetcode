...
int main(){
    ...
    // setup
    auto    largestnumber   {std::numeric_limits<int>::min()};
    auto    numsset         {std::set<int>(nums.begin(), nums.end())};
    auto    myheap          {std::vector<int>()};
    std::make_heap(myheap.begin(), 
                   myheap.end(), 
                   std::greater<int>());

    // going through the histogram
    for(const auto x: numsset){

        // adding element to it
        myheap.push_back(x);
        largestnumber = std::max(largestnumber, x);

        // making a heap out of it
        std::push_heap(myheap.begin(), 
                       myheap.end(), 
                       std::greater<int>());

        // popping if size is greater
        if (myheap.size() > 3){
            std::pop_heap(myheap.begin(), 
                          myheap.end(), 
                          std::greater<int>());
            myheap.pop_back();
        }
    ...
    int finaloutput = largestnumber;
    if (myheap.size() == 3){
        std::pop_heap(myheap.begin(), 
                      myheap.end(), 
                      std::greater<int>());    
        finaloutput = myheap.back();
    }
