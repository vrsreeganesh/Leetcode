...
int main(){
    ...
    // setup
    auto    pipe        {std::deque<TreeNode*>()};
    pipe.push_back(root);
    const   auto&   root_value     {root->val};
    auto    final_output    {true};

    // bfs
    while   (pipe.size()!=0)
    {
        // popping front-value
        const   auto&   curr_root       {pipe.front()};
        pipe.pop_front();
        const   auto&   curr_value      {curr_root->val};

        // checking if current-value is different from root-value
        if (curr_value != root_value)   {final_output = false; break;}

        // adding children to pipe
        if(curr_root->left)             {pipe.push_back(curr_root->left);}
        if(curr_root->right)            {pipe.push_back(curr_root->right);}
    }
