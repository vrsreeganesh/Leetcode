...
int main(){
    ...
    // input- configuration
    auto    n   {static_cast<int>(13)};

    // setup
    auto    length_of_square_numbers    {static_cast<std::size_t>(1+std::ceil(std::sqrt(n)))};
    auto    square_numbers              {std::vector<int>(length_of_square_numbers, 0)};

    // filling up the vector
    for(auto i = 0; i < square_numbers.size(); ++i)
        square_numbers[i]   =   i*i;

    // building the dptable
    auto    dptable     {std::vector<int>(n+1)};
    dptable[0] = 1;

    // building the dp-table
    for(auto i = 1; i <= n; ++i){

        // consider previous answer
        auto&   curr_value      {i};

        // checking if the current-value can be achieved with just the coins
        if(std::find(square_numbers.begin(),
                     square_numbers.end(),
                     curr_value)    != square_numbers.end())    {dptable[i] = 1; continue;}

        // considering all the previous guys
        auto    min_num_coins   {std::numeric_limits<int>::max()};
        for(int j = i-1; j > (i-1)/2; --j)
        {
            // getting current dp-entry
            const   auto   var00   {j};
            const   auto    var01   {curr_value - j};

            // checking what coin to use to get here
            const   auto    num_coins_required      {dptable[var00] + dptable[var01]};

            // calculating number of coins required
            min_num_coins   =   std::min(num_coins_required, min_num_coins);
        }

        // writing to entry
        dptable[i] = min_num_coins;
    }
