...
int main(){
    ...
    // input- configuration
    auto nums   {vector<int>{1,-2,3,-2}};

    // setup
    int global_maximum      {nums[0]};
    int global_minimum      {nums[0]};
    int running_max         {0};
    int running_min         {0};
    int total               {0};
    ...
    for(auto x: nums){
        ...
        // calculating running maximum and minimum values
        running_max = max(running_max + x, x);
        running_min = min(running_min + x, x);

        // running up the total value
        total += x;

        // updating global maximum and minumum values
        global_maximum = max(global_maximum, running_max);
        global_minimum = min(global_minimum, running_min);
    }

    // some condition
    if (global_maximum > 0)
        global_maximum = max(global_maximum, total - global_minimum);
