...
int main(){
    ...
    // setup
    TreeNode *finalanswer {static_cast<TreeNode*>(nullptr)};
    std::function<int(TreeNode*)> foo = [&foo,
                           &finalanswer,
                           &p, &q](
        TreeNode* root){
        ...
        if (finalanswer != nullptr)         {return 0;}

        // searching left
        auto leftsum     = foo(root->left);
        auto rightsum    = foo(root->right);

        // summing up values 
        auto sumvalues = leftsum + rightsum;
        if (root == p || root == q)         {sumvalues+=1;}

        // updating final answer
        if (sumvalues == 2 && 
            finalanswer == nullptr)         {finalanswer = root;}
    ...
    cout << format("Ancestor-value = {}, Ancestor-address = {}\n", 
        finalanswer->val, static_cast<void*>(finalanswer));
