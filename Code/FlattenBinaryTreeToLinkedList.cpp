...
int main(){
    ...
    // recursion lambda
    auto foo = [&foo](TreeNode*             root, 
                      vector<TreeNode*>&    nodeaddresses){

        // sending it back
        if (root == nullptr) return;

        // adding address 
        nodeaddresses.push_back(root);

        // going down left
        if (root->left) foo(root->left, nodeaddresses);
        if (root->right) foo(root->right, nodeaddresses);
    ...
    // fill registers
    foo(root, nodeaddresses);

    // going through the addresses and reconnecting it
    for(int i = 0; i<nodeaddresses.size()-1; ++i){
        nodeaddresses[i]->left  = nullptr;
        nodeaddresses[i]->right = nodeaddresses[i+1];
    }

    // taking care of the last
    nodeaddresses[nodeaddresses.size()-1]->left     = nullptr;
    nodeaddresses[nodeaddresses.size()-1]->right    = nullptr;
