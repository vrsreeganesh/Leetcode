...
void foo(vector<int>            pathsofar,
    ...
    // checking legnth of pathsofar
    if (pathsofar.size() > k) return;

    // adding the current path to the final output if the length is k
    if (pathsofar.size() == k)  {finalOutput.push_back(pathsofar); return;}

    // setting up starting point
    int startingpoint;
    if (pathsofar.size() == 0)  startingpoint = 0+1;
    else                        startingpoint = pathsofar[pathsofar.size()-1]+1;

    // running through the options
    for(int i = startingpoint; i<=n; ++i){

        // calling the graph on those
        auto pathsofar_temp = pathsofar;
        pathsofar_temp.push_back(i);

        // calling the functiona again
        foo(pathsofar_temp, n, k, finalOutput);
    }
