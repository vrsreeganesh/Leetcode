...
int main(){
    ...
    // setup
    if (head == nullptr)    {cout << format("empty-list\n"); return 0;}

    // setup
    ListNode* traveller = nullptr;
    vector<pair<int, ListNode*>> nodeaddresses;

    // goign through the list
    traveller = head;
    while(traveller){

        // adding it ot the list
        nodeaddresses.push_back(std::pair<int, ListNode*>({traveller->val, traveller}));
    ...
    std::sort(nodeaddresses.begin(),
              nodeaddresses.end(),
              [](pair<int, ListNode*> a, 
                 pair<int, ListNode*> b)    {return a < b;});

    // reconnecting
    for(int i = 0; i<nodeaddresses.size()-1; ++i)
        nodeaddresses[i].second->next = nodeaddresses[i+1].second;


    // making the last one connect to nullptr
    nodeaddresses[nodeaddresses.size()-1].second->next = nullptr;

    // returning top of the list
    fPrintLinkedList("finalOutput = ", nodeaddresses[0].second);
