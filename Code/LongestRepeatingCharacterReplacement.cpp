...
int main(){
    ...
    auto left                   {0};
    auto right                  {0};
    auto num_characters_local   {0};
    auto finalOutput            {0};     
    auto maxelement             {-1};                                       

    std::vector<int> characterCount(26, 0);                                     // histogram
    
    // going through the elements
    for(right = 0; right < s.size(); ++right){

        // adding to count 
        characterCount[static_cast<int>(s[right])-65] += 1;
        ...
        // calculating longest 
        finalOutput = std::max(finalOutput, right-left+1);
