...
int main(){
    ...
    for(auto x: tokens){
        // check if the string is an operation
        if (x == "+" || x == "-" || x == "*" || x == "/"){
            // gettting current operation
            operation = x;
            int     output;

            // getting values in stack
            if (operation == "+")       output = temp[temp.size()-2] + temp[temp.size()-1];
            else if (operation == "-")  output = temp[temp.size()-2] - temp[temp.size()-1];
            else if (operation == "*")  output = temp[temp.size()-2] * temp[temp.size()-1];
            else if (operation == "/")  output = temp[temp.size()-2] / temp[temp.size()-1];

            // writing the outputs to the stack
            temp.pop_back();
            temp.pop_back();
            temp.push_back(output);
        }
        else{
            int xtemp = std::stoi(x);       // converting to values
            temp.push_back(xtemp);          // converting to numerical values
        }
    }
