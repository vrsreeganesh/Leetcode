...
int main(){
    ...
    auto carry   {1};
    auto sum    {-1};
    vector<int> finalOutput(digits.size()+1, 0);

    // goign through the numbers
    for(int i = 0; i<digits.size(); ++i){
        ...
        finalOutput[finalOutput.size()-1-i] = sum;
    }

    // in case there is one carry left
    if (carry != 0)     {finalOutput[0] = 1;}
    else                {finalOutput = vector<int>(finalOutput.begin()+1, finalOutput.end());}

    // sending back the finaloutput
