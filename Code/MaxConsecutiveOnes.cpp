...
int main(){
    ...
    // input- configuration
    auto nums   {vector<int>{1,1,0,1,1,1}};
    ...
    for(int i = 0; i<nums.size(); ++i){
        if (nums[i] == 1){
            if (p1 == -1)   {p1 = i;}                       // updating p1
            auto temp   {i - p1 + 1};                       // calculating size so far
            finaloutput = std::max(temp, finaloutput);      // updating finaloutput
        }
        else    {p1 = -1;}
    }
