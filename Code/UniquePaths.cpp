...
int main(){
    ...
    // setup
    auto row    = [&n](int x){return x/n;};
    auto col    = [&n](int x){return x%n;};
    auto tid    {m*n - 1};

    // building dp-table
    vector<vector<int>> dptable;
    for(int i = 0; i<m; ++i)    {dptable.push_back(vector<int>(n, 0));}
    dptable[m-1][n-1] = 1;
    
    // moving through the thing
    while(tid>=0){
        
        auto currcol        {col(tid)}; 
        auto currrow        {row(tid)};
        auto nummethods     {dptable[currrow][currcol]};
        
        if (currcol < n-1)  {nummethods += dptable[currrow][currcol+1];}
        if (currrow < m-1)  {nummethods += dptable[currrow+1][currcol];}
        
        dptable[currrow][currcol] = nummethods;
