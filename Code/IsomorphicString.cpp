...
int main(){
    ...
    std::unordered_set<char>    destinations;
    string                      r;

    // going through string
    for(int i = 0; i<s.size(); ++i){

        // checking if two strings are different
        if (s[i] != t[i]){
            // checking if we already have a mapping for this
            if (mapping.find(s[i]) == mapping.end()){

                // before we addd, we need to check if t[i] is already used as a destination
                if (destinations.find(t[i]) != destinations.end()) {
                ...
                // adding new mapping
                mapping[s[i]] = t[i];
                destinations.insert(t[i]);
            }
            r.push_back(mapping[s[i]]);
        }
        else{
            // checking if we already have a mapping for this
            if (mapping.find(s[i]) == mapping.end()){
                ...
                // adding new mapping
                mapping[s[i]] = s[i];
                destinations.insert(s[i]);
            }
            r.push_back(mapping[s[i]]);
        }

        // if what we have so far is different, we're sending it back
        string s_subset(t.begin(), t.begin() + r.size());
        if(s_subset != r) {cout << format("final-output = false \n"); return 0;}
