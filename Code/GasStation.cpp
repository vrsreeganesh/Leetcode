...
int main(){
    ...
    // setup
    auto acc    {0};                                                // variable to accumulate values
    vector<int> diffvec;                                            // to store differences
    auto temp   {0};                                                // to maintain values in cache
    int finaloutput {-1};                                           // the final output sotring var

    // running through it
    for(int i = 0; i<cost.size(); ++i){
        temp    = gas[i] - cost[i];                                 // calculating flux
        acc     += temp;                                            // appending to integral flux
        diffvec.push_back(temp);                                    // storing instantial-flux to vector
    }
    if (acc<0)  {finaloutput = -1; return 0;}                       // if total flux is zero, the task cannot be completed

    // going through the diff-vec
    acc = 0;
    for(int i = 0; i<diffvec.size(); ++i){
        acc += diffvec[i];                                          // accumulating flux
        if (acc<0)  {acc = 0; finaloutput = i+1;}                   // updating start-point (since we cannot go below zero)
    }
