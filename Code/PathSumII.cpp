         ...
         vector<vector<int>>&   finalOutput)
{
    ...
    // adding current-value to running vector
    runningvector.push_back(root->val);
    runningsum += root->val;

    // checking if current-value has reached target sum
    if (runningsum  == targetSum && 
        root->left  == nullptr && 
        root->right == nullptr){
        finalOutput.push_back(runningvector); return;
    }

    // going down left and right
    foo(root->left, targetSum, runningsum, runningvector, finalOutput);
    foo(root->right, targetSum, runningsum, runningvector, finalOutput);
...
int main(){
    ...
    foo(root, targetSum, runningsum, runningvector, finalOutput);
