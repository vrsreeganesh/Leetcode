...
{
    TreeNode*   node;
    unsigned long long         branch_path;
    int         curr_level;
};
...
int main(){
    ...
    // setup
    auto    pipe    {std::deque<PipeContent>()};
    auto    levels  {std::vector<std::vector<unsigned long long>>()};

    // filling pipe
    pipe.push_back({root, 0, 0});
    
    // running bfs
    while(pipe.size() != 0)
    {
        // popping front
        auto    front_entry     {pipe.front()}; pipe.pop_front();
        auto&   curr_node       {front_entry.node};
        auto&   branch_path     {front_entry.branch_path};
        auto&   curr_level      {front_entry.curr_level};

        // adding contents to the levels structure
        if(curr_level == levels.size())   {levels.push_back(std::vector<unsigned long long>());}
        levels[curr_level].push_back(branch_path);

        // adding children to pipe
        if (curr_node->left)    {pipe.push_back({curr_node->left,   branch_path*2 + 0, curr_level+1});}
        if (curr_node->right)   {pipe.push_back({curr_node->right,  branch_path*2 + 1, curr_level + 1});}
    }

    // calculating largest size
    auto    finaloutput     {std::numeric_limits<int>::min()};
    for(const   auto&   x: levels){
        // fetching numbers from branch paths
        auto    starting_point      {x[0]};
        auto    ending_point        {x[x.size()-1]};

        // calculating final output
        finaloutput     =   std::max(finaloutput, static_cast<int>(ending_point - starting_point + 1));
    }
