         ...
         int&       minlevel)
{
    ...
    if (root->left == nullptr && root->right == nullptr){
        minlevel = std::min(minlevel, currlevel);
        return;
    }

    // calling functions
    foo(root->left,     currlevel,  minlevel);
    foo(root->right,    currlevel,  minlevel);
...
int main(){
    ...
    // setup
    auto currlevel  {0};
    auto minlevel   {std::numeric_limits<int>::max()};

    // calling the function
    foo(root, currlevel, minlevel);

    // returning the minlevel
