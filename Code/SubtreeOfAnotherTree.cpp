...
// main-file ===================================================================
bool fAreWeTheSame(TreeNode* root, TreeNode* subroot){

    // checking if root-note values are the same
    if (root == nullptr && subroot == nullptr)      return true;
    if (root == nullptr || subroot == nullptr)      return false;
    if (root->val != subroot->val)                  return false;

    // going through the branches
    auto isLeftBranchEqual      {fAreWeTheSame(root->left,      subroot->left)};
    auto isRightBranchEqual     {fAreWeTheSame(root->right,     subroot->right)};

    // returning
    return (isLeftBranchEqual && isRightBranchEqual);
}

void f(TreeNode* root, TreeNode* subroot, bool& logicAccum){

    // base-case
    if (root == nullptr || subroot == nullptr)      return;

    // checking if they have the same head-value
    if (root->val == subroot->val)
        logicAccum = logicAccum || fAreWeTheSame(root, subroot);

    // sending it back if we found one already
    if (logicAccum) return;

    // moving onto the next
    f(root->left,       subroot, logicAccum);
    f(root->right,      subroot, logicAccum);
...
int main(){
    ...
    auto subRoot        {new TreeNode(4)};
    subRoot->left       = new TreeNode(1);
    subRoot->right      = new TreeNode(2);
    
    // setup
    bool logicAccum = false;
    f(root, subRoot, logicAccum);
