...
int fRob1(std::vector<int> nums){    
    ...
    for(int i = 1; i<nums.size(); ++i){
        ...
        // checking which max value to use
        if (i > 1)      {maxCandidate = std::max(nums[i]+ dp[i-2], dp[i-1]);}
        else            {maxCandidate = std::max(nums[i], dp[i-1]);}

        // storing max-candidate
        dp.push_back(maxCandidate);

    }

    // returning the mx-value
    return dp[dp.size()-1];
}
...
int main(){
    ...
    // sending things back
    if(nums.size()== 0)     {cout << format("final-output = 0\n"); return 0;}
    if(nums.size()== 1)     {cout << format("final-output = {}\n", nums[0]); return 0;}

    // setup
    auto firstCandidate     {fRob1(std::vector<int>(nums.begin(), nums.end()-1))};
    auto secondCandidate    {fRob1(std::vector<int>(nums.begin()+1, nums.end()))};

    // returning value
    auto finaloutput        {std::max(std::max(firstCandidate, secondCandidate), nums[0])};
