...
int main(){
    ...
    std::function<void(const TreeNode*, int)> f = [&f, &finalOutput](
        const TreeNode* root,
        int             level){
        ...
        while (finalOutput.size()<level+1)
            finalOutput.push_back(std::vector<int>());

        // appending to value
        if (finalOutput.size() < level+1)
            finalOutput[level] = std::vector<int>(root->val);
        else
