...
void foo(TreeNode*                                  root,
         map<int, vector<int>, std::greater<int>>&  levelToMembers,
         int                                        currlevel){

    // sending it back
    if (root==nullptr) return;

    // incrementing currlevel
    ++currlevel;

    // moving left
    foo(root->left, levelToMembers, currlevel);

    // checking if the current-level is present
    if (levelToMembers.find(currlevel) == levelToMembers.end())
        levelToMembers[currlevel] = vector<int>({root->val});
    else
        levelToMembers[currlevel].push_back(root->val);

    // moving right
    foo(root->right, levelToMembers, currlevel);
...
int main(){
    ...
    foo(root, levelToMembers, -1);

    // printing the values
    #pragma omp parallel for
    for(auto x: levelToMembers)     {finalOutput.push_back(x.second);}
