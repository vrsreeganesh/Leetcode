...
void fTraverse(TreeNode*            root,
               std::vector<int>&    finalView, 
               int                  count){
    
    // base-case
    if (root == nullptr)                        {return;}

    // increasing size of finalView incase it is not big enough
    while (finalView.size() < count+1)          {finalView.push_back(-INT_MAX);}

    // assigning values
    finalView[count] = root->val;

    // writing just the right value
    if (root->left  != nullptr)                 {fTraverse(root->left, finalView, count +1);}
    
    // going down this branch
    if (root->right != nullptr)                 {fTraverse(root->right, finalView, count+1);}
...
int main(){
    ...
    // checking something simple
    if (root == nullptr)                        {cout << format("final-output = {}\n", std::vector<int>()); return 0;}
    
    // method to print this out
    std::vector<int> finalView;
    auto count  {0};

    // calling the function that traverses through these things. 
    finalView.push_back(root->val);
    fTraverse(root, finalView, 0);
