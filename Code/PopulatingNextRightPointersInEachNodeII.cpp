...
int main(){
    ...
    // running
    if (root == nullptr) {cout << format("done!\n"); return 0;}

    // calling the function
    int count = -1;
    vector< vector<Node*> > addressvectors;

    // calling th efunction
    foo(root, count, addressvectors);

    // building the connections
    for(auto x: addressvectors){
        int i = 0;
        for(; i<x.size()-1; ++i)
            if (x[i] != nullptr)    {x[i]->next = x[i+1];}                      // look to the right 

        // ensuring that the last pointer points to thr right most 
        x[i]->next = nullptr;
    }
