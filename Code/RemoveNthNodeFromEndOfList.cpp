...
int main(){
    ...
    // trivial case
    if (!head) {cout << format("final-output = "); fPrintLinkedList(head); cout << endl;}
    ...
    ListNode* delayedTraveller  = prehead;
    
    // going through the list
    while(traveller){
        if (++nodecounter > n)  {delayedTraveller = delayedTraveller->next;}
        traveller = traveller->next;
    }

    // reconnecting
    delayedTraveller->next = delayedTraveller->next->next;
