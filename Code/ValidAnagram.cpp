...
int main(){
    ...
    // building count-vector for both
    for(auto x: s) ++s_count[static_cast<int>(x)-97];
    for(auto x: t) ++t_count[static_cast<int>(x)-97];

    // comparing the two
    for (int i = 0; i<s_count.size(); ++i) {

        // element-wise checking
        if (s_count[i] != t_count[i]) {cout << format("final-output = false \n");}
