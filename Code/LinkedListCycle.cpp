...
int main(){
    ...
    // trivial case
    if (head == nullptr) {cout << format("final-output = false\n"); return 0;}

    // setup
    ListNode* dummy     = new ListNode(0);
    dummy->next         = head;
    ListNode* fastboi   = dummy;
    ListNode* slowboi   = dummy;
    ...
    while (fastboi != nullptr && fastboi->next != nullptr) {
        ...
        if (slowboi == fastboi) {cout << format("final-output = true\n"); return 0;}
    }

    // since exiting the list implies you left
