...
int main(){
    ...
    auto calculatearea = [](vector<int> points1, vector<int> points2, vector<int> points3){

        // calculating the area
        int firstcomponent       {points1[0] * (points2[1] - points3[1])};
        int secondcomponent      {points2[0] * (points3[1] - points1[1])};
        int thirdcomponent       {points3[0] * (points1[1] - points2[1])};
        double finaloutput       {static_cast<double>(0.5 * std::abs(firstcomponent + secondcomponent + thirdcomponent))};
    ...
    for(int i = 0; i<points.size(); ++i){
        for(int j = i+1; j<points.size(); ++j){
            for(int k = j+1; k<points.size(); ++k){
                finaloutput = std::max(temp, calculatearea(points[i], points[j], points[k]));
            }
        }
    }
