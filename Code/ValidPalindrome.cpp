...
int main(){
    ...
    // setup
    auto pleft          {0};
    auto pright         {static_cast<int>(s.size())-1};
    auto finaloutput    {true};

    // lambda to check if alphanumeric
    auto isalphanumeric = [](const int& x){
        if (x-65 >= 0 && 90-x  >= 0)      {return true;}      // upper-case check
        if (x-97 >= 0 && 122-x >= 0)      {return true;}      // lower-case check
        if (x-48 >= 0 && 57-x  >= 0)      {return true;}      // numeric check
        return false;
    };
    ...
    while(pleft < s.size() && pright >= 0){
        ...
        while(pleft  < s.size() && isalphanumeric(s[pleft]) == false)    {++pleft;}
        while(pright >= 0       && isalphanumeric(s[pright]) == false)   {--pright;}
