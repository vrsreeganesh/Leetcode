...
int main(){
    ...
    // lambda to check
    auto    checkDiagonal   =   [&matrix](int row, int col){

        // storing the init-value
        auto    init_value      {matrix[row][col]};
        ++row;  ++col;

        while(row < matrix.size() && col < matrix[0].size()){
            if (matrix[row][col] != init_value)     {return false;}
            ++row; ++col;
        }
        return true;
    };
    ...
    for(auto col = 0; col < matrix[0].size(); ++col)
        finaloutput     =   finaloutput && checkDiagonal(0, col);

    // going down 
    for(auto row = 0; row < matrix.size(); ++row)
        finaloutput     =   finaloutput && checkDiagonal(row, 0);
