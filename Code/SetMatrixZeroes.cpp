...
int main(){
    ...
    // setup
    vector<bool> colregister(matrix[0].size(),  false); colregister.reserve(colregister.size());
    vector<bool> rowregister(matrix.size(),     false); rowregister.reserve(rowregister.size());
    ...
    for(const auto& x: matrix){
        for(const auto& y: x)   {cout << format("{}, ", y);}
        cout << format("\n");
    }
