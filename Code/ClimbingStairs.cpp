...
int main(){
    ...
    // number of steps from the last step to the last
    dpTable.push_back(0);
    dpTable.push_back(1);
    dpTable.push_back(2);

    // finding number of ways this can be done
    for(int i = 2; i<n; ++i){
        // adding up the last two values
        dpTable.push_back(dpTable[dpTable.size() - 1] + dpTable[dpTable.size() - 2]);
    }
