...
int main(){
    ...
    // going through the values from the bottom right to left
    for(int i = grid.size()*grid[0].size()-1; i>=0; --i){

        // converting numbers to the row--index and colindex
        auto rowindex   {i/static_cast<int>(grid[0].size())};
        auto colindex   {i%static_cast<int>(grid[0].size())};

        // finding sum of the children with present
        auto smallestchild  {std::numeric_limits<int>::max()};

        // checking the bottom left
        if (rowindex == grid.size()-1 && colindex == grid[0].size()-1)          {smallestchild = 0; }
        else if(rowindex == grid.size()-1)                                      {smallestchild = mingrid[rowindex][colindex+1]; }
        else if (colindex == grid[0].size()-1)                                  {smallestchild = mingrid[rowindex+1][colindex];}
        else                                                                    {smallestchild = min(mingrid[rowindex+1][colindex], 
                                                                                                        mingrid[rowindex][colindex+1]);}

        // storing to the final results
        mingrid[rowindex][colindex] = mingrid[rowindex][colindex] + smallestchild;
