...
{    
    // checking size
    if (runningvec.size() == maxnumelements)    {finalOutput.push_back(runningvec);return;} 
    
    // checking the next set of options
    for(int i = currindex+1; i < nums.size(); ++i){
        
        // creating arguments for next call
        auto runningvec_temp    {runningvec}; 
        runningvec_temp.push_back(nums[i]);
        foo(finalOutput, nums, i, runningvec_temp, maxnumelements);
...
int main(){
    ...
    // setup
    vector<vector<int>> finalOutput {vector<int>({})};
    
    // calling the function 
    #pragma omp parallel for
    for(int i = 1; i<=nums.size(); ++i)
        foo(finalOutput, nums, -1, vector<int>({}), i);
