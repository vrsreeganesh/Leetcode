...
void foo(int                numopens, 
    ...
    // sending it back 
    if (numopens > n || numcloses > n) return;
    else if (numopens == n && numcloses == n) {finalOutput.push_back(runningstring); return;}
    
    // opening route 
    foo(numopens+1, numcloses, n, runningstring + "(", finalOutput);
    
    // closing route
    if (numopens>numcloses)
        foo(numopens, numcloses+1, n, runningstring+")", finalOutput);
...
int main(){
    ...
    // trivial case
    if (n == 0) {cout << format("finalOutput = []\n"); return 0;}

    // calling the function
    vector<string> finalOutput;
    foo(0, 0, n, "", finalOutput);
