...
void foo(TreeNode*                  root,
    ...
    foo(root->left, nodeaddresses, nodevalues);
    ...
    // going right
    foo(root->right, nodeaddresses, nodevalues);
...
int main(){
    ...
    foo(root, nodeaddresses, nodevalues);

    // sort the values and assign it
    std::sort(nodevalues.begin(), nodevalues.end());

    // writing the values as is
    #pragma omp
    for(int i = 0; i<nodeaddresses.size(); ++i){
        nodeaddresses[i]->val = nodevalues[i];
    }
