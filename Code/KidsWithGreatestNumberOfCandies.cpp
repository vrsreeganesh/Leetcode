...
int main(){
    ...
    // setup
    auto iter = std::max_element(candies.begin(), candies.end()); // finding max value
    vector<bool> finaloutput;

    // going through all values
    for(int i = 0; i<candies.size(); ++i){
        if ((candies[i]+extraCandies) >= *iter) {finaloutput.push_back(true);}
        else                                    {finaloutput.push_back(false);}
    }
