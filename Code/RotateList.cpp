...
int main(){
    ...
    // trivial cases
    if (head == nullptr) {fPrintLinkedList("head = ", head); return 0;}

    // storing the starting point
    auto startingpoint {head};
    auto listsize      {0};

    // connecting the last-node with the first
    auto traveller {head};
    while(traveller->next) {traveller = traveller->next; ++listsize;}
    ++listsize;
    traveller->next = head;

    // to get rid of cycles
    k = k % listsize; 
    auto cutoffpoint    {listsize - k};
    auto count          {0};
    auto finalresult    {static_cast<ListNode*>(nullptr)};

    // finding the cutting off point
    traveller = head;
    while(traveller){
        ++count;                                                                // appending count
        if (count == cutoffpoint){
            finalresult = traveller->next;                                      // setting up the new head
            traveller->next = nullptr;                                          // setting up last pointer
            break;                                                              // breaking up
        }
        traveller = traveller->next;                                            // moving to the next point
    }

    // printing the linked list
    fPrintLinkedList("final-result = ", finalresult);
