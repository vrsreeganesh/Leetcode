         ...
         vector<vector<int>>&   finalOutput)
{
    ...
    int runningsum  = std::accumulate(runningvector.begin(), 
                                      runningvector.end(), 
                                      0);
    
    // sending it back
    if (runningsum > target)        {return;}
    if (runningsum == target)       {finalOutput.push_back(runningvector); return;}
    
    // going through the different options
    for(auto x: candidates){
        
        // checking if this should be 
        if (runningvector.size() != 0 && x < runningvector[runningvector.size()-1]) {continue;}
        
        auto temp {runningvector}; 
        temp.push_back(x);

        // recursive-call
        foo(candidates, temp, target, finalOutput);
    }
...
int main(){
    ...
    foo(candidates, runningvector, target, finalOutput);
