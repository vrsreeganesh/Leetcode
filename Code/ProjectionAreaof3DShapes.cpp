...
int main(){
    ...
    // setup
    auto finaloutput    {0};
    auto xzvector   {vector<int>(grid.size(), 0)};
    auto yzvector   {vector<int>(grid[0].size(), 0)};

    auto maxheightvalues    {vector<int>{}};
    for(int row = 0; row<grid.size(); ++row){
        for(int col = 0; col<grid[0].size(); ++col){

            // calculating xy-counts
            if (grid[row][col] != 0)    {++finaloutput;}

            // updating xzvector
            xzvector[row]   = std::max(xzvector[row], grid[row][col]);

            // updating yzvector
            yzvector[col]   = std::max(yzvector[col], grid[row][col]);

        }
    }

    // calculating sums
    finaloutput = std::accumulate(xzvector.begin(), xzvector.end(), finaloutput);
    finaloutput = std::accumulate(yzvector.begin(), yzvector.end(), finaloutput);
