...
int main(){
    ...
    while(explorer < nums.size()){
        ...
        while(explorer < nums.size() && nums[explorer] == 0)  {explorer++;}

        // copying value
        if (explorer<nums.size() && anchor <nums.size())
            nums[anchor++]    = nums[explorer++];
    }

    // zeroing out the rest 
    while(anchor < nums.size()) {nums[anchor++] = 0;}

    // printing the finaloutput 
    cout << format("finaloutput = "); fPrintVector(nums);
