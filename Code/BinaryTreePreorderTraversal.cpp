...
// main-file ===================================================================
void foo(TreeNode* root, vector<int>& finaloutput){
    // sending it back
    if (root == nullptr) return;

    // adding current-value to the final output
    finaloutput.push_back(root->val);

    // going left
    foo(root->left, finaloutput);
    foo(root->right, finaloutput);
