...
int main(){
    ...
    // setting up pipe
    std::deque<TreeNode*>   pipe;
    pipe.push_back(root);
    auto    last_value      {std::numeric_limits<int>::min()};

    // recursive lambda
    std::function<void(const TreeNode*, const int)>     InOrderTraversal = [
        &InOrderTraversal,
        &values](const     TreeNode*       root,
                      const     int             parent_value)
    {
        const   auto&   curr_value      {root->val};
        if(root->left)      {InOrderTraversal(  root->left, curr_value);}
        values.push_back(curr_value);
        if(root->right)      {InOrderTraversal( root->right, curr_value);}
    };

    // calling the function
    InOrderTraversal(root, std::numeric_limits<int>::min());

    // calculating final output
    finaloutput = std::transform_reduce(
