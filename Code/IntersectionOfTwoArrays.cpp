...
int main(){
    ...
    // input- configuration
    auto nums1  {vector<int>{4,9,5}};
    auto nums2  {vector<int>{9,4,9,8,4}};
    
    // setup
    std::set<int> nums1set(nums1.begin(), nums1.end());
    std::set<int> nums2set(nums2.begin(), nums2.end());

    // printing
    cout << format("nums1set = {}\n", nums1set);
    cout << format("nums2set = {}\n", nums2set);

    // calculating the intersection
    vector<int> finaloutput;
    for(const auto& x: nums1set){

        if (std::find(nums2set.begin(),
                      nums2set.end(),
                      x) != nums2set.end()) {finaloutput.push_back(x);}
    }
