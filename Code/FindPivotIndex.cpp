...
int main(){
    ...
    // setup
    vector<int> numsleft(nums.size(), 0);
    vector<int> numsright(nums.size(), 0);
    int         sumleft     {0};
    int         sumright    {0};
    int         leftindex   {0};
    int         rightindex  {0};
    ...
    for(int i = 0; i<nums.size(); ++i){
        ...
        // fetching indices
        leftindex   = i;
        rightindex  = nums.size()-1-i;

        // accumulating left and right values
        sumleft     += nums[leftindex]; 
        sumright    += nums[rightindex];

        if (leftindex < nums.size()-1)    {numsleft[leftindex+1] = sumleft;}
        if (rightindex > 0)            {numsright[rightindex-1] = sumright;}
    }
    ...
    // printing 
    cout << format("numsleft = ");  fPrintVector(numsleft);
    cout << format("numsright = "); fPrintVector(numsright);
