...
int main(){
    ...
    ListNode* lefttraveller     = lefthead;
    ListNode* righthead         = new ListNode(-1);
    ListNode* righttraveller    = righthead;
    
    // going through the list
    while(traveller){
        // checking next-value
        if (traveller->val < x){
            lefttraveller->next     = new ListNode(traveller->val);
            lefttraveller           = lefttraveller->next;
        }
        else {
            righttraveller->next    = new ListNode(traveller->val);
            righttraveller          = righttraveller->next;
        }
    ...
    // connecting
    lefttraveller->next = righthead->next;

    // returning 
    fPrintLinkedList("Final-output = ", lefthead->next);
