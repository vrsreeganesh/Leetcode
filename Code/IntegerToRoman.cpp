...
int main(){
    ...
    string  finaloutput;                                                        // variable to hold the final output
    int     count;                                                              // variable that will hold the counts
    auto mulstring = [](const int& count, 
                        const string& inputstring, 
                        string& finaloutput){
        if (count == 0) {return;}
        for(int i = 0; i<count; ++i){finaloutput += inputstring;}
    };                                                                          // lambda-function for int * string multiplications (python style)
    ...
    for(int i = numToString.size()-1; i>=0; --i){
        ...
        num     = num - numToString[i].first*count;
