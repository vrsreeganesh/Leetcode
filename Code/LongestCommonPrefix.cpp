...
int main(){
    ...
    // setup
    int p               {0};                                                    // index-pointer for boundary
    int runcondition    {true};                                                 // breaking condition
    string prefix;
    ...
    while(runcondition){
        ...
        if (p >= strs[0].size())            {++p; runcondition = false; break;}

        // checking if this candidate
        for(int i = 1; i<strs.size(); ++i){

            // checking if valid
            if (p >= strs[i].size())        {runcondition = false; break;}
    ...
    // subsetting and printing the prefix
    prefix = string(strs[0].begin(), strs[0].begin()+p-1); 
