...
int main(){
    ...
    // setup
    int p1          {0};                                    // index-pointer to buying
    int p2          {0};                                    // index-pointer to selling
    int accprofit   {0};                                    // variable to accumulate profit
    int currprofit  {std::numeric_limits<int>::min()};      // variable to hold curr-profit

    // going through this
    while(p2 < prices.size()){

        currprofit = prices[p2] - prices[p1];               // calculating current profit

        if (currprofit > 0){
            accprofit   += currprofit;                      // accumulating the profit
            p1          = p2++;                             // moving the starting point
            continue;                                       // moving into the next iteration
        }
