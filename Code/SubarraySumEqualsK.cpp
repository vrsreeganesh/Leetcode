...
int main(){
    ...
    // input-configuration
    auto nums   {vector<int>{6,4,3,1}};
    auto k      {10};
    
    // setup
    auto leftSum    {vector<int>(nums.size())};
    std::partial_sum(nums.begin(), nums.end(), leftSum.begin());

    // calculating final-output
    auto finaloutput    {0};
    finaloutput += std::count(leftSum.begin(),  leftSum.end(),  k);

    // moving from left to right
    for(int i = 0; i < leftSum.size(); ++i){
        for(int j = i+1; j < leftSum.size(); ++j){
            auto sumvalue   {leftSum[j] - leftSum[i]};
            if (sumvalue == k)  {++finaloutput;}
        }
    }
