...
int main(){
    ...
    // input- configuration
    auto nums   {vector<int>{3,4,-1,1}};
    
    // in the off-chance that minvalue is one, building ordered set of values
    std::set<int> orderedvalues;
    for(auto x: nums)               {if (x>0) orderedvalues.insert(x);}
    
    // going through the ordered set
    auto finaloutput    {1};
    auto flag           {0};
    for(const auto& x: orderedvalues)
        if (x - finaloutput++ != 0) {--finaloutput; break;}
    
    // printing based on flag value
