...
int main(){
    ...
    auto heap               {std::vector(nums.begin(), nums.begin()+k)};
    std::make_heap(heap.begin(), heap.end());
    auto numbertobedeleted  {-1};
    auto numbertobeadded    {-1};
    auto topvalue           {-1};
    finaloutput.push_back(*(std::max_element(heap.begin(), heap.end())));

    // going through the nums
    for(int i = 1; i<=nums.size()-k; ++i){

        // removing the element that just left the window
        numbertobedeleted   = nums[i-1];
        auto deletion_it    = std::find(heap.begin(), 
                                        heap.end(), 
                                        numbertobedeleted);
        heap.erase(deletion_it);
        std::make_heap(heap.begin(), 
                       heap.end()); 
        
        // adding new number to heap
        numbertobeadded     = nums[i+k-1];
        heap.push_back(numbertobeadded); 
        std::push_heap(heap.begin(), 
                       heap.end());

        // picking biggest number
        ...
        std::push_heap(heap.begin(), 
                       heap.end());
        finaloutput.push_back(topvalue);
    }
