...
// main-file ===================================================================
// traversing function
void fRecursive(const std::vector<int>&                 nums,           \
                std::vector< std::vector<int> >&        finalOutput,    \
                const std::vector<int>                  path,           \
                const std::vector<int>                  numsleft){

    // check if length is reached
    if (path.size() == nums.size()){
        
        // add to final output
        if (std::find(finalOutput.begin(), finalOutput.end(), path) == finalOutput.end()) 
            finalOutput.push_back(path);

        // returning since the path is complete
        return;
    } 

    // choosing from numsleft
    for(int i = 0; i<numsleft.size(); ++i){

        // choosing a number, 
        auto num                {numsleft[i]};
        auto pathtemp           {path};
        pathtemp.push_back(num);

        // making a copy of nums left, removing character
        auto numsleft_temp      {numsleft};
        numsleft_temp.erase(numsleft_temp.begin() + i);

        // calling function
        fRecursive(nums, finalOutput, pathtemp, numsleft_temp);
    }
...
int main(){
    ...
    fRecursive(nums, finalOutput, path, numsleft);
