...
int main(){
    ...
    // setup
    StopWatch timer;                                            // timer-object
    int p0          {0};                                        // first index-pointer
    int p1          {1};                                        // second index-pointer
    int maxprofit   {0};                                        // variable to hold max-profit
    int curr        {-1};                                       // variable to hold current-profit

    // going through array
    while(p1<prices.size()){
        curr        = prices[p1] - prices[p0];                  // calculating current profit
        maxprofit   = curr > maxprofit ? curr : maxprofit;      // updating max-profit
        if (curr < 0)   {p0 = p1;}                              // updating p0 if we find lower point
        ++p1;
    }
