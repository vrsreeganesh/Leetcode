...
int main(){
    ...
    // input- configuration
    auto    mat     {std::vector<std::vector<int>>({
        {1,2},
        {3,4}
    })};
    auto    r       {1};
    ...
    // lambda for converting tid to row and columns
    auto    coordsToTID = [&mat](auto input_vector){
        return input_vector[0] * mat[0].size() + input_vector[1];
    };
    auto    tidToCoords =   [&c](auto   tid){
        return std::vector<int>({static_cast<int>(tid/c), static_cast<int>(tid%c)});
    };

    // going through the mat
    for(auto row = 0; row < mat.size(); ++row){
        for(auto col = 0; col < mat[0].size(); ++col){
            auto target_coords  {tidToCoords(row*mat[0].size() + col)};
            canvas[target_coords[0]][target_coords[1]] = mat[row][col];
        }
    }
