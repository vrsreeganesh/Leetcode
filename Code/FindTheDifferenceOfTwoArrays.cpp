...
int main(){
    ...
    for(const auto& x: set1){
        if(set2.find(x) == set2.end())  {firstinput.push_back(x);}
    }

    // filling second entry
    for(const auto& x: set2){
        if (set1.find(x) == set1.end()) {secondinput.push_back(x);}
    }

    // pushing to the final output
    finaloutput.push_back(firstinput);
    finaloutput.push_back(secondinput);

    // printing 
    cout << format("first input = "); fPrintVector(firstinput);
    cout << format("second input = ");fPrintVector(secondinput);
