...
int main(){
    ...
    // setting up lambdas
    auto counttwos      = [](int x){
        
        // in case of x == 0 or x being odd
        if(x%2 != 0 || x == 0)  {return 0;}

        // counting number of 2s we can pluck from this number
        auto numtwos {0};
        while(true){
            if (x%2 == 0)   {++numtwos; x /= 2; continue;}
            else            {return numtwos;}
        }
    ...
    auto countfives     = [](int x){
        
        // in case of x == 0 or x not being a multiple of five
        if(x%5 != 0 || x == 0)          {return 0;}

        // counting number of 5s we can pluck from this number
        auto numfives   {0};
        while(true){
            if (x%5 == 0)   {++numfives; x /= 5; continue;}
            else            {return numfives;}
        }
    ...
    // calculating numtwos and numfives
    auto numtwos    {0};
    auto numfives   {0};
    ...
    for(int i = n; i>=0; --i){
        ...
        numtwos     += counttwos(i);
        numfives    += countfives(i);

    }

    // calculating numzeros
    auto finaloutput    {std::min(numtwos, numfives)};
