...
int main(){
    ...
    // setup
    auto fCalculateDistance = [](const vector<int>& coordinate_A,
                                 const vector<int>& coordinate_B){

        auto xdif           {std::abs(coordinate_A[0] - coordinate_B[0])};
        auto ydif           {std::abs(coordinate_A[1] - coordinate_B[1])};
        auto common         {std::min(xdif, ydif)};
        auto distleft       {std::max(xdif, ydif) - std::min(xdif, ydif)};
        return distleft + common;
    };

    // calculating distances
    auto finaloutput    {std::inner_product(points.begin(),
                                            points.end()-1,
                                            points.begin()+1,
                                            0,
                                            std::plus<int>(),
