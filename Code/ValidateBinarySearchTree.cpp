...
int main(){
    ...
    // setup
    bool validBSTFlag = true;
    vector<int> nodevalues;

    std::function<void(const TreeNode*)> foo = [&foo, &nodevalues, &validBSTFlag](
        const TreeNode* root){
        
        // if false, stop everything
        if (validBSTFlag == false)      {return;}
        ...
        // adding current
        if (nodevalues.size() == 0)     {nodevalues.push_back(root->val);} 
        else{
            // check top 
            auto topvalue = nodevalues[nodevalues.size()-1];

            // check if top value is less than or equal 
            if (topvalue >= root->val)  {validBSTFlag = false;}                 // since this is not valid
            else                        {nodevalues.push_back(root->val);}
        }
