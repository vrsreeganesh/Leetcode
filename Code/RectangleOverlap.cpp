...
int main(){
    ...
    // input- configuration
    auto rec1   {vector<int>{4,4,14,7}};
    auto rec2   {vector<int>{4,3,8,8}};
    
    // checking if all points in rec2 are to the right of xcoordinates
    const auto& top1        {rec1[3]};
    const auto& bottom1     {rec1[1]};
    const auto& left1       {rec1[0]};
    const auto& right1      {rec1[2]};

    const auto& top2        {rec2[3]};
    const auto& bottom2     {rec2[1]};
    const auto& left2       {rec2[0]};
    const auto& right2      {rec2[2]};

    // comparing
    bool finaloutput    {true};
    if (right1 < left2)         {finaloutput = false;}
    else if(top1 < bottom2)     {finaloutput = false;}
    else if(bottom1 > top2)     {finaloutput = false;}
    else if(right2 < left1)     {finaloutput = false;}
